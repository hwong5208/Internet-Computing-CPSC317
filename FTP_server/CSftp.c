...
int sendFile(int peer, FILE *f);

int sendPath( int  peer, char * file , uint32_t offset);
...
int main(int argc, char **argv) {
   ...
   struct sockaddr_in server,client;
    ...
    if(serverSock ==-1){
        perror(" Error: Failed t create socket. \n ");
        return -1;
    }
       ...
       if(strcasecmp(command,"USER")==0){
           ...
           if(loggedIn ==0){
               ...
               //    sendStr(sock,"230 - Login successful\n" );
                   ...
                   message = "430 - This server only supports the username: cs317\n";
                   write(sock, message, strlen(message));
               }
           }else{
               message = "430 - Can't change from cs317\n";
               write(sock, message, strlen(message));
           }
           ...
           if(loggedIn ==1 ){
               ...
               if(passiveMode ==1){
                   ...
                   if(pasvPort > 1024 && pasvPort <= 65535 && pasvSock>= 0){
                       ...
                       int st = sendPath(dataClient, parameter, 0);
